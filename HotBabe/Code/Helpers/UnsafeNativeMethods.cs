...
namespace HotBabe.Code.Helpers
{
  ...
  public static class UnsafeNativeMethods
  {
    ...
    ///<summary>
    /// Return true if the active application is full screen
    ///</summary>
    ///<returns></returns>
    public static bool IsActiveWindowFullscreen()
    {
      ...
      Process process = Process.GetProcessById(GetWindowProcessID(activeWindowHandle));
      ...
      if (module != null &&
          module.ModuleName.Equals("explorer.exe",
                                   StringComparison.InvariantCultureIgnoreCase))
      {
        return false;
      }
      WINDOWINFO info = new WINDOWINFO();
      if (GetWindowInfo(activeWindowHandle, ref info))
      {
        Size monitorSize = SystemInformation.PrimaryMonitorSize;
        return (monitorSize == info.rcWindow.Size &&
                info.rcWindow.Equals(info.rcClient) &&
                info.cbSize == 0 &&
                info.cxWindowBorders == 0 &&
                info.cyWindowBorders == 0);
      }
      return false;
    }
    ...
    ///</summary>
    ///<param name="m"></param>
    public static void ReDrawWindow(Message m)
    {
